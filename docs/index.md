...
| zfs_snapshot_limit       | Number                        | Sets the number of ZFS snapshots a VM can take. Undefined or null indicates no limits, and setting null disables previously-set limits.  NOTE: On compute nodes running versions of SmartOS prior to June, 2020, setting zfs_snapshot_limit to null will cause the VM to not allow snapshots at all. Setting a high number will mitigate this on older versions of SmartOS.             | No                  | Yes    | Yes    |
