...
function cnapiThenUfds(vmapi, server, uuid) {
    function cnapiMachineLoad(vmapi, server, uuid, callback) {
    ...
    function task(callback) {
        return cnapiMachineLoad(vmapi, server, uuid, callback);
    }

    vmapi.queue.push(task);
}
...
function processHeartbeat(vmapi, server, hb) {
    ...
    vmapi.cache.getState(uuid, function (err, state) {
        if (err) {
        ...
        if (state) {
            ...
            if (oldZoneState != zoneState || oldLastModified < lastModified)
                return cnapiThenUfds(vmapi, server, uuid);

        } else {
            var newState = zoneState + ';' + hb[8];

            vmapi.cache.setState(uuid, newState, function (anErr) {
                if (anErr) {
                    vmapi.log.error('Error caching VM state %s %s', uuid);
                    return;
                }

                vmapi.log.debug('VM state cached %s %s', uuid, newState);
                return cnapiThenUfds(vmapi, server, uuid);
            });
        }
    });
