...
Heartbeater.prototype.processHeartbeat = function (vmapi, server, hb, cb) {
    ...
    vmapi.moray._getVmObject(uuid, onGetVm);

    function onGetVm(err, vm) {
        if (err && err.name === 'ObjectNotFoundError') {
            log.trace({ vm_uuid: uuid }, 'VM does not exist on Moray %s', uuid);
            self.invalidateCache(vmapi, uuid, hb, server, true, true, cb);
        } else if (err) {
            log.error({ err: err, vm_uuid: uuid },
                'Error getting VM from moray');
            cb(err);
        } else {
            self.processState(vmapi, vm, uuid, hb, server, cb);
        }
    }
};
...
function (vmapi, vm, uuid, hb, server, cb) {
    var log = this.log;
    var self = this;
    var lastMod = new Date(hb['last_modified']);

    // Current VM state. state and last modified are used to check if machine
    // needs to be updated, server uuid is used to check if we have a duplicate
    //
    // A new VM will typically have the following values:
    // - state: provisioning
    ...
    var oldLastMod;
    if (vm['last_modified'] !== undefined) {
        oldLastMod = new Date(vm['last_modified']);
    }
    // If server was not set then this is a new VM
    var oldServer = vm['server_uuid'] || server;

    log.trace({ vm_uuid: uuid },
        'Timestamps for VM %s, oldLastModified: %s, lastModified: %s',
        uuid, oldLastMod, lastMod);

    if (server === oldServer && oldState === hb.state && oldLastMod &&
        oldLastMod.getTime() === lastMod.getTime()) {
        return cb();
    } else if (server !== oldServer) {
        log.error({ vm_uuid: uuid }, 'Found duplicate VM UUIDs. Server from ' +
            ...
            oldServer, server, oldState, oldLastMod.toISOString(), hb);
        return cb();
    }

    var newVm, callCnapi;
    if (oldLastMod === undefined || oldLastMod < lastMod) {
        newVm = false;
        callCnapi = true;
    ...
    return self.invalidateCache(vmapi, uuid, hb, server, newVm, callCnapi, cb);
};
...
function (vmapi, uuid, hb, server, newMachine, callCnapi, cb) {
    ...
    if (callCnapi === false) {
        self.updateStateOnMoray(vmapi, uuid, hb, server, newMachine, onError);
        return;
    }
