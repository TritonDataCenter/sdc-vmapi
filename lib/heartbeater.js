...
function Heartbeater(options) {
    ...
    // five seconds
    this.duplicateVms = {};
    // Generalized for processing heartbeat errors
    this.errorVms = {};
    ...
    this.queue.drain = function () {
        ...
        dtrace._rstfy_probes['heartbeat-queue-drain'].fire(function () {
            return ([ self.heartbeatQueueSize ]);
        });
    };
...
Heartbeater.prototype.processHeartbeats = function (vmapi, server, vms) {
    ...
    function fireQueueProbe() {
        dtrace._rstfy_probes['heartbeat-queue-push'].fire(function () {
            return ([ self.queue.length(), self.heartbeatQueueSize ]);
        });
    }
    ...
    for (var i = 0; i < uuids.length; i++) {
        ...
        if (uuid !== 'global' && self.processing[uuid] === undefined) {
            var dtraceId = fireProbe(server, uuid, hb);
            ...
            self.log.trace({ vm_uuid: uuid }, 'Ignoring heartbeat from VM %s, ' +
...
function (vmapi, uuid, hb, server, newMachine, callCnapi, dtraceId, cb) {
    ...
    function onError(err, message) {
        if (err) {
            ...
            var now = new Date();
            var before = self.errorVms[uuid];

            // Only log every this.logInterval seconds for a specific VM
            if (before === undefined ||
                ((now - before)/1000) > self.logInterval) {
                self.errorVms[uuid] = now;
