...
Heartbeater.prototype.processHeartbeats = function (vmapi, server, vms) {
    ...
    if (vmapi.config.useVmAgent) {
        self.log.debug('Skipping getMissingUuids, using vm-agent');
        return;
    }
...
function (vmapi, uuid, hb, server, newMachine, callCnapi, dtraceId, cb) {
    ...
    if (callCnapi === false) {
        ...
        self.updateNicsInNapi(vmapi, uuid, hb, newMachine);
        return;
    }
 ...
 * Update NICs in NAPI
 */
Heartbeater.prototype.updateNicsInNapi =
function (vmapi, uuid, hb, newMachine) {
    var log = this.log;
    ...
    vmapi.napi.updateNicsState(uuid, hb.state, function (err) {
        if (err) {
            log.error(err, 'Error updating NICs for VM ' + uuid);
        } else {
            log.debug({ vm_uuid: uuid }, 'NICs updated for VM %s', uuid);
        }
    });
};
