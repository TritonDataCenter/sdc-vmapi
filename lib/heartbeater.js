...
Heartbeater.prototype.listen = function (vmapi, cb) {
    ...
    self.on('heartbeat', function (serverUuid, hbs, missing) {
        vmapi.cnapi.getServer(serverUuid, function (err, server) {
            ...
            // If CNAPI knows about the server we can proceed to process its
            // heartbeats
...
function processDestroyed(vmapi, uuid) {
    ...
    function onGetVm(morayErr, oldVm) {
        if (morayErr) {
            vmapi.log.error({ err: morayErr, vm_uuid: uuid },
    ...
    function onMorayDestroyed(err, oldVm) {
        if (err) {
            vmapi.log.error({ err: err, vm_uuid: oldVm.uuid },
                'Error marking % as destroyed', oldVm.uuid);
        } else {
            vmapi.log.info({ vm_uuid: oldVm.uuid },
...
function invalidateCache(vmapi, uuid, hb, server, newMachine, callCnapi) {
    ...
    function onMoray(err, vmapi2, uuid2, hb2, server2) {
        return updateState(vmapi2, uuid2, hb2, null, onError);
    }
...
function updateStateOnMoray(vmapi, uuid, hb, server, newMachine, cb) {
    if (newMachine) {
        vmapi.log.debug({ vm_uuid: uuid },
            'New VM %s, skipping updateState', uuid);
...
function cnapiGetVm(vmapi, uuid, hb, server, cb) {
    ...
    function onGetVm(err, vm) {
        if (err) {
            ...
            vmapi.log.error({ vm_uuid: uuid, server: server },
...
function updateVmOnMoray(vmapi, uuid, hb, vm, cb) {
    ...
    function onPutVm(err) {
        if (err) {
            ...
            vmapi.log.debug({ vm_uuid: uuid },
                'VM object %s updated on moray', uuid);
            cb(null, vmapi, uuid, hb, vm);
        }
    }
}
 ...
 * Add NICs to NAPI
 */
function addNicsToNapi(vmapi, uuid, hb, vm, cb) {
    vmapi.napi.addNics(vm, function (err) {
        if (err) {
            vmapi.log.error({ err: err, vm_uuid: vm.uuid },
                'Error adding NICs for VM %s', vm.uuid);
            cb(err);
        } else {
            vmapi.log.debug({ vm_uuid: vm.uuid },
                'NICs added for VM %s', vm.uuid);
            cb(null, vmapi, uuid, hb, vm);
        }
    });
}
...
function updateState(vmapi, uuid, hb, vm, cb) {
    ...
    function onSetState(err) {
        if (err) {
            ...
            vmapi.log.debug({ vm_uuid: uuid }, 'VM %s state cached', uuid, hb);
            cb(null, vmapi, vm);
        }
    }
}
