...
function processHeartbeat(vmapi, server, hb) {
    ...
    function onGetState(err, stateTimestamp) {
        if (err) {
            vmapi.log.error({ err: err, uuid: uuid },
        ...
        if (stateTimestamp) {
            ...
            invalidateCache(vmapi, uuid, hb, server, true);
        }
    }
}
...
function processState(vmapi, stateTimestamp, uuid, hb, server) {
    ...
    if (oldState != hb.state ||
        oldLastModified < lastModified) {
        invalidateCache(vmapi, uuid, hb, server, false);
    }
}
 ...
 */
function invalidateCache(vmapi, uuid, hb, server, newMachine) {
    vmapi.cache.setState(uuid, hb, onSetState);

    function onSetState(err) {
        if (err) {
            vmapi.log.error({ err: err, uuid: uuid },
                'Error caching VM state before moray update');
            cb(err);
        } else {
            vmapi.log.debug('VM state cache before moray update', hb);
            waterfall();
        }
    }
    ...
    function onWaterfallError(err, result) {
        if (err) {
            vmapi.log.error({ err: err, uuid: uuid },
                'Error invalidating cache state of VM');
            // vmapi.cache.delState(uuid, function (err2) {
            //     if (err2) {
            //         vmapi.log.error({ err: err2, uuid: uuid },
            //             '(fallback) Error deleting state from cache');
            //     } else {
            //         vmapi.log.info('(fallback) VM ' + uuid +
...
function cnapiGetVm(vmapi, uuid, hb, server, cb) {
    ...
    function onGetVm(err, vm) {
        if (err) {
            vmapi.log.error({ err: err, uuid: uuid }, 'Error talking to CNAPI');
            cb(err);
        } else if (vm) {
...
function updateState(vmapi, uuid, hb, vm, cb) {
    ...
    function onSetState(err) {
        if (err) {
            vmapi.log.error({ err: err, uuid: uuid }, 'Error caching VM state');
            cb(err);
        } else {
