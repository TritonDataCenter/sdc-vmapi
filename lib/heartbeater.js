...
function Heartbeater(options) {
    ...
    // This object let us keep track of which duplicate VM UUIDs have been
    // detected. We use it to keep track what was the last time a log.error
    // message was printed and avoid flooding the system with alarms every
    // five seconds
    this.duplicateVms = {};
    ...
    // an initial heartbeat delay (i.e. 8 means try again in the next 8
    // heartbeats) and a maximum delay where we stop trying to process that
    // heartbeat. At that point we would need to refresh the VM data with
    // /vms?sync=true. Sample errorVms object:
    //
    // { '01bddd61-2581-418d-bfe9-d0e40187880f': { current: 2, next: 16 } }
    //
    // When heartbeater gives up trying to process a heartbeat (maxDelay was
    // set to a value), then an object would look like this:
    //
    // { '<uuid>': { current: 16, next: 16, giveUp: true } }
    //
    // This means: the current heartbeat count is 2 and in the 16h heartbeat we
    // process it expecting we don't get any more failures. Every heartbeat is
    // received every ~5 seconds.
    //
    this.errorVms = {};
    this.retry = options.retry || { initialDelay: 8 };
    if (this.retry.maxDelay === undefined) {
        this.retry.maxDelay = Infinity;
    }
    this.logInterval = 600; // 10 minutes
 ...
 */
Heartbeater.prototype.listen = function (vmapi) {
    var self = this;
...
/*
 * Decides if a heartbeat with a previous error should be processed
 */
Heartbeater.prototype.shouldIgnoreHeartbeat = function (uuid) {
    var previous = this.errorVms[uuid];

    if (this.processing[uuid] === true)
        return true;
    if (previous === undefined)
        return false;

    if (previous.giveUp === true) {
        return true;
    } else {
        previous.current++;
        return (previous.current !== previous.next);
    }
};


/*
 * Removes a VM marked as "with errors" when its heartbeats are trying to be
 * processed
 */
Heartbeater.prototype.removeErrorVm = function (uuid) {
    delete this.errorVms[uuid];
};
...
Heartbeater.prototype.processHeartbeats = function (vmapi, server, vms) {
    ...
    var uuid, hb, previous;
    for (var i = 0; i < uuids.length; i++) {
        uuid = uuids[i];
        hb = vms[uuid];
        previous = this.errorVms[uuid];

        self.log.trace({ server: server, vm_uuid: uuids[i] },
            'Heartbeat from %s:', uuids[i], vms[uuids[i]]);

        if (uuid !== 'global' && self.shouldIgnoreHeartbeat(uuid) === false) {
            ...
            if (self.processing[uuid] === true) {
                self.log.debug({ vm_uuid: uuid }, 'Ignoring heartbeat from ' +
                    'VM %s, already processing another heartbeat', uuid, hb);
            } else if (previous) {
                if (previous.giveUp) {
                    self.log.info({ vm_uuid: uuid },
                        'Gave up retrying for VM %s', uuid);
                } else {
                    self.log.debug({ vm_uuid: uuid },
                        'Ignoring heartbeat from VM %s, current retry count ' +
                        'is %d, will process at %d', uuid, previous.current,
                        previous.next);
                }
            }
        }
...
function (vmapi, stateTimestamp, uuid, hb, server, dtraceId, cb) {
    ...
    if (oldServer !== undefined && server !== oldServer) {
        var now = new Date();
        var before = this.duplicateVms[uuid];

        // Only log every this.logInterval seconds
        if (before === undefined || ((now - before)/1000) > this.logInterval) {
            this.duplicateVms[uuid] = now;
            log.error({ vm_uuid: uuid }, 'Found duplicate VM UUIDs. Server ' +
                'from cached data: %s, server from heartbeat: %s, cached ' +
                'status and timestamp: %s;%s, new heartbeat data: %j',
...
function (vmapi, uuid, hb, server, newMachine, callCnapi, dtraceId, cb) {
    ...
    function onError(err, message) {
        if (err) {
            ...
            var previous = self.errorVms[uuid];
            if (previous === undefined) {
                previous = {
                    current: 0,
                    next: self.retry.initialDelay
                };
                self.errorVms[uuid] = previous;
            } else {
                // At this point previous.current == previous.next
                // If we hit the maxDelay allow it
                if (previous.current === self.retry.maxDelay) {
                    previous.giveUp = true;
                } else {
                    previous.next = previous.next * 2;
                }
            }
            log.debug({ vm_uuid: uuid }, 'Retry attempt #%d for VM %s',
                previous.current, uuid);
...
Heartbeater.prototype.cnapiGetVm = function (vmapi, uuid, hb, server, cb) {
    ...
    function onGetVm(err, vm) {
        if (err) {
            ...
            cb(err2, 'Unexpected response received from CNAPI');
        }
    }
};
