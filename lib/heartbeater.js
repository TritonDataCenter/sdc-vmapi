...
Heartbeater.prototype.getMissingUuids = function (cache, server, hbs, cb) {
    ...
    function onCache(err, cached) {
        if (err) {
            /*JSSTYLED*/
            self.log.error('Could not get list of cached VM UUIDs for server %s', server);
            cb(err);
            return;
        }
    ...
    function onSaveCache(err, missing){
        if (err) {
            /*JSSTYLED*/
            self.log.error('Could not cache list VM UUIDs for server %s', server);
            cb(err);
            return;
        }
...
function processDestroyed(vmapi, uuid) {
    ...
    function onMorayDestroyed(err, oldVm) {
        if (err) {
            /*JSSTYLED*/
            vmapi.log.error('Error marking ' + oldVm.uuid + 'as destroyed on moray', err);
        } else {
            vmapi.log.info('VM ' + oldVm.uuid + ' marked as destroyed on moray');
        }
    }
