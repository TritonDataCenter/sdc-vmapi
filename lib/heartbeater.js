...
function Heartbeater(options) {
    ...
    // Ths array helps us find which machines are no longer on a server and
    // call ufds.updateMachine
    this.lastSeenUuids = [];
 ...
 * missing UUIDs we can assume the machine was destroyed and mark as destroyed
 * on UFDS.
 */
 ...
 */
function processHeartbeats(zapi, server, hbs, missing) {
    var self = this;
    var ufds = zapi.ufds;
    var napi = zapi.napi;
    var cnapi = zapi.cnapi;
    var cache = zapi.cache;
    ...
    for (var i = 0; i < missing.length; i++) {
        var oldMachine = cache.get(missing[i]);

        if (oldMachine) {
            ufds.markAsDestroyed(cache, oldMachine, function (err) {
                if (err) {
