...
function invalidateCache(vmapi, uuid, hb, server, newMachine) {
    ...
    function onSetState(err) {
        if (err) {
            /*JSSTYLED*/
            vmapi.log.error('Error caching VM state before moray update %s', uuid, err);
            cb(err);
        } else {
    ...
    function onWaterfallError(err, result) {
        if (err) {
            vmapi.log.error('Error invalidating cache of VM %s', uuid, err);
            vmapi.cache.delState(uuid, function (err2) {
                if (err2) {
                    vmapi.log.error('(fallback) Error deleting ' + uuid +
                        ' state from cache', err2);
                } else {
...
function updateStateOnMoray(vmapi, uuid, hb, server, newMachine, cb) {
    ...
    function onUpdate(err) {
        if (err) {
            vmapi.log.error('Error updating VM %s state on moray', uuid, err);
            cb(err);
        } else {
