...
function attemptConnect(cb) {
    ...
    function onReady() {
        ...
        function onJson(message, headers, deliveryInfo) {
            ...
            self.log.debug({ server: serverUuid }, 'Heartbeat received from %s',
...
Heartbeater.prototype.listen = function (vmapi, cb) {
    ...
    self.on('heartbeat', function (serverUuid, hbs, missing) {
        vmapi.cnapi.getServer(serverUuid, function (err, server) {
            if (err) {
...
Heartbeater.prototype.processHeartbeats = function (vmapi, server, vms) {
    ...
    for (var i = 0; i < uuids.length; i++) {
        ...
        self.processHeartbeat(vmapi, server, vms[uuids[i]]);
    }
 ...
 */
Heartbeater.prototype.processHeartbeat = function (vmapi, server, hb) {
    var log = this.log;
    var self = this;
    var uuid = hb.uuid;
    ...
    function onGetState(err, stateTimestamp) {
        ...
        if (stateTimestamp) {
            self.processState(vmapi, stateTimestamp, uuid, hb, server);
        } else {
...
function (vmapi, stateTimestamp, uuid, hb, server) {
    var log = this.log;
    var self = this;
    ...
    if (oldLastModified < lastModified) {
        self.invalidateCache(vmapi, uuid, hb, server, false, true);
    } else if (oldState != hb.state) {
        self.invalidateCache(vmapi, uuid, hb, server, false, false);
    } // else do nothing
};
...
function (vmapi, uuid, hb, server, newMachine, callCnapi) {
    var log = this.log;
    var self = this;
    ...
    function onMoray(err, vmapi2, uuid2, hb2, server2) {
        return self.updateState(vmapi2, uuid2, hb2, null, onError);
    }
    ...
    function onError(err, result) {
        if (err) {
            ...
            if (vmapi.status !== vmapi.statuses.OK) {
...
function (vmapi, uuid, hb, server, newMachine, cb) {
    ...
    if (newMachine) {
        log.debug({ vm_uuid: uuid }, 'New VM %s, skipping updateState', uuid);
 ...
 */
Heartbeater.prototype.updateVmOnMoray = function (vmapi, uuid, hb, vm, cb) {
    var log = this.log;
 ...
 * Add NICs to NAPI
 */
Heartbeater.prototype.addNicsToNapi = function (vmapi, uuid, hb, vm, cb) {
    var log = this.log;
 ...
 */
Heartbeater.prototype.updateState = function (vmapi, uuid, hb, vm, cb) {
    var log = this.log;
 ...
 */
Heartbeater.prototype.cleanupCache = function (vmapi, vm, cb) {
    var log = this.log;
    ...
    function onGetVm(err, cached) {
        if (err) {
            log.error({ err: err, vm_uuid: vm.uuid },
                'Error fetching VM %s state from cache', vm.uuid);
            cb(err);
        }
    ...
    function onDelVm(err) {
        if (err) {
            log.error({ err: err, vm_uuid: vm.uuid },
                'Error removing VM %s state from cache', vm.uuid);
            cb(err);
        } else {
            log.debug({ vm_uuid: vm.uuid },
                '"provisioning" state of VM %s removed from cache', vm.uuid);
            cb(null);
        }
    }
};
