    ...
    var queue = this.queue = async.queue(function (task, callback) {
        ...
        task.run(function (err) {
            ...
            if (err) {
                ...
                setTimeout(function () {
                    self.retryHeartbeat(task.vmapi, task.server,task.heartbeat);
                }, 5000);
...
Heartbeater.prototype.retryHeartbeat = function (vmapi, server, hb) {
    ...
    function logAttempt(log, host) {
        function _log(number, delay) {
            ...
            if (lastError.name && lastError.name === 'ResourceNotFoundError') {
                log[level]({
                    ip: host,
                    attempt: number,
                    delay: delay
    ...
    var retry = backoff.call(processFn, function (err) {
        ...
        log.info('Heartbeat successfully processed after %d attempts',attempts);
    });
