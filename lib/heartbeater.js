...
function (vmapi, vm, uuid, hb, server, cb) {
    ...
    log.trace({ vm_uuid: uuid },
        'Timestamps for VM %s, oldLastMod: %s, lastModified: %s',
        uuid, oldLastMod, lastModified);

    if (server === oldServer && oldState === hb.state &&
        oldLastMod === lastModified) {
        return cb();
