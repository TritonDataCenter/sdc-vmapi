...
function validType(type) {
  return VALID_MACHINE_TYPES.indexOf(type) != -1;
}
...
function validateMachine(params) {
  ...
  if (!params.type)
    throw new restify.MissingParameterError('Machine type is required');

  if (!validType(params.type))
    throw new restify.InvalidArgumentError('%s is not a valid machine type',
                                            params.type);
