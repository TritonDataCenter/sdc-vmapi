...
function machineToUfds(m) {
    ...
    machine.uuid = m.zonename || m.uuid;
    machine.owner_uuid = m.owner_uuid;
    machine.brand = m.brand;

    machine.ram = m.max_physical_memory || m.ram;
    machine.swap = m.max_swap || m.swap;
    machine.disk = m.quota || m.disk;
    ...
    machine.status = m.state || m.status;
    ...
    if (machine.cpucap)
        machine.cpucap = m.cpucap || m.cpu_cap;
