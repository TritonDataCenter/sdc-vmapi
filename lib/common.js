...
function validateParams(ufds, params, callback) {
    if (!params.owner_uuid)
        callback(new restify.MissingParameterError('Owner UUID is required'));

    if (!validUUID(params.owner_uuid))
        callback(new restify.ConflictError('Owner UUID is not a valid UUID'));

    if (!params.dataset_uuid)
        callback(new restify.MissingParameterError('Dataset UUID is required'));

    if (!validUUID(params.dataset_uuid))
        callback(new restify.ConflictError('Dataset UUID is not a valid UUID'));
    ...
    if (!validBrand(params.brand))
        callback(new restify.InvalidArgumentError('%s is not a valid ' +
                                               'machine brand', params.brand));

    if (!params.ram)
        callback(new restify.MissingParameterError('Machine RAM is required'));
    ...
    if (params.networks && !validUUIDs(params.networks))
        callback(new restify.InvalidArgumentError('\'%s\' are not valid ' +
                                      'UUIDs for Networks', params.networks));

    if (params.alias && !validAlias(params.alias))
        callback(new restify.ConflictError('Alias ' + params.alias +
                                           ' is invalid'));
