/*
 * Copyright (c) 2012, Joyent, Inc. All rights reserved.
 *
 * Main entry-point for the VMs API.
 */

var UFDS = require('sdc-clients').UFDS;
var EventEmitter = require('events').EventEmitter;
var sprintf = require('sprintf').sprintf;
var assert = require('assert');
var restify = require('restify');
var util = require('util');

var common = require('./../common');

var SUFFIX = 'o=smartdc';

var USERS = 'ou=users, ' + SUFFIX;
var USER_FMT = 'uuid=%s, ' + USERS;
var VM_FMT = 'vm=%s, ' + USER_FMT;



/*
 * UFDS Constructor
 */
function Ufds(options) {
    EventEmitter.call(this);
    this.log = options.log;
    this.options = options;
    this.connected = false;

    this.connect();
}

util.inherits(Ufds, EventEmitter);



/*
 * Attempts to connect to UFDS, retrying until connection is established
 * emits 'ready' event when connection to UFDS becomes available
 */
Ufds.prototype.connect = function () {
    this.log.info('Connecting to UFDS...');
    attemptConnect.call(this);
};



function attemptConnect() {
    var self = this;
    var timeout = null;
    this.connection = new UFDS(this.options);

    function onReady() {
        clearTimeout(timeout);
        timeout = null;
        self.connected = true;

        self.log.info('Connected to UFDS...');
        self.emit('ready');
    }

    function onError(err) {
        self.log.error('Error connecting to UFDS');
        self.log.info('Re-attempting connection to UFDS');

        self.connection.removeAllListeners();
        self.connected = false;
        self.connection = null;

        if (!timeout) {
            attemptConnect.call(self);
        }
    }

    function timeoutCallback() {
        attemptConnect.call(self);
    }

    this.connection.on('ready', onReady);
    this.connection.on('error', onError);
    timeout = setTimeout(timeoutCallback, 10000);
}



/*
 * Handles reconnection when a call to LDAP indicates that the connection has
 * been lost
 */
function reconnectionHandler(err) {
    if (err.message == 'connect ECONNREFUSED' ||
        err.message == 'no connection') {
        this.connected = false;
        attemptConnect.call(this);
    }
}


/*
 * LDAP Search
 */
Ufds.prototype.search = function (base, options, callback) {
    var self = this;

    return this.connection.search(base, options, function (err, items) {
        if (err) {
            reconnectionHandler.call(self, err);
            callback(err);
            return;
        }

        callback(null, items);
    });
};



/*
 * LDAP Add
 */
Ufds.prototype.add = function (dn, entry, callback) {
    var self = this;

    return this.connection.add(dn, entry, function (err) {
        if (err) {
            reconnectionHandler.call(self, err);
            callback(err);
            return;
        }

        callback(null);
    });
};



/*
 * LDAP Modify
 */
Ufds.prototype.modify = function (dn, entry, callback) {
    var self = this;

    return this.connection.modify(dn, entry, function (err) {
        if (err) {
            reconnectionHandler.call(self, err);
            callback(err);
            return;
        }

        callback(null);
    });
};



/*
 * LDAP Del
 */
Ufds.prototype.del = function (dn, callback) {
    var self = this;

    return this.connection.del(dn, function (err) {
        if (err) {
            reconnectionHandler.call(self, err);
            callback(err);
            return;
        }

        callback(null);
    });
};



/*
 * Adds a vm on UFDS. This function expects a cnapi-like vm object.
 * This object is converted to a UFDS like vm schema
 */
Ufds.prototype.addVm = function (vm, callback) {
    var dn = sprintf(VM_FMT, vm.uuid, vm.owner_uuid);

    var newVm = common.vmToUfds(vm);
    delete newVm.owner_uuid;
    newVm.objectclass = 'vm';

    this.add(dn, newVm, function (err) {
        if (err) {
            callback(err);
        } else {
            callback(null);
        }
    });
};



/*
 * Updates a vm on UFDS. This function expects a cnapi-like vm object.
 * This object is converted to a UFDS like vm schema. For now this function
 * is doing a complete replace of the vm object properties
 */
Ufds.prototype.replaceVm = function (vm, callback) {
    var dn = sprintf(VM_FMT, vm.uuid, vm.owner_uuid);

    var newVm = common.vmToUfds(vm);
    delete newVm.owner_uuid;

    var operation = {
        type: 'replace',
        modification: newVm
    };

    this.modify(dn, operation, function (err) {
        if (err) {
          callback(err);
        } else {
          callback(null);
        }
    });
};



/*
 * Updates vm attributes on UFDS
 */
Ufds.prototype.updateVm = function (vm, params, callback) {
    var dn = sprintf(VM_FMT, vm.uuid, vm.owner_uuid);

    var updateParams = common.vmToUfds(params);
    delete updateParams.owner_uuid;

    var operation = {
        type: 'replace',
        modification: updateParams
    };

    this.modify(dn, operation, function (err) {
        if (err) {
          callback(err);
        } else {
          callback(null);
        }
    });
};



/*
 * Deletes a vm from UFDS
 */
Ufds.prototype.deleteVm = function (vm, callback) {
    var dn = sprintf(VM_FMT, vm.uuid, vm.owner_uuid);

    this.del(dn, function (err) {
        if (err) {
          callback(err);
        } else {
          callback(null);
        }
    });
};



/*
 * Adds or 'updates' a vm on UFDS. Currently it is completely replacing the
 * vm attributes but it will only update attributes that have changed
 */
Ufds.prototype.addUpdateVm = function (vm, cb) {
    if (!this.connected) {
        this.log.error('Cannot update VM data without connection to UFDS');
        return;
    }

    var self = this;
    var log = this.log;

    var params = {
        uuid: vm.uuid,
        owner_uuid: vm.owner_uuid
    };

    function add() {
        function onAdd(err) {
            if (err) {
                log.error('Could not create vm on UFDS', err);
                cb(err);
            } else {
                log.debug('Added vm ' + vm.uuid + ' to UFDS');
                cb(null);
            }
        }

        self.addVm(vm, onAdd);
    }

    function update(oldVm) {
        var diff = common.objectDiff(oldVm, vm);

        function onUpdate(err) {
            if (err) {
                log.error('Could not update vm on UFDS', err);
                cb(err);
            } else {
                log.debug('VM updated ' + vm.uuid + ' on UFDS');
                cb(null);
            }
        }

        self.updateVm(vm, diff, onUpdate);
    }

    function onGetVm(err, old) {
        if (err) {
            log.error('Error getting vm info from UFDS', err);
            cb(err);
        } else {
            if (old) {
                update(old);
            } else {
                add();
            }
        }
    }

    self.getVm(params, onGetVm);
};



/*
 * Gets a vm from UFDS. When a vm is found, the second argument will
 * have an object, otherwise it will be null
 */
Ufds.prototype.getVm = function (params, callback) {
    var baseDn;
    var uuid = params.uuid;
    var owner_uuid = params.owner_uuid;

    if (!common.validUUID(uuid))
        return callback(
          new restify.InvalidArgumentError('VM UUID is not a valid UUID'));

    if (owner_uuid) {
        if (!common.validUUID(owner_uuid))
            return callback(
              new restify.InvalidArgumentError('Owner UUID is not a ' +
                'valid UUID'));

        baseDn = sprintf(USER_FMT, owner_uuid);
    } else {
        baseDn = USERS;
    }

    var options = {
        scope: 'sub',
        filter: '(&(objectclass=vm)(uuid=' + uuid + '))'
    };

    return this.search(baseDn, options, function (err, items) {
        if (err) {
            callback(err);
            return;
        }

        if (items.length == 0) {
            callback(null, null);
        } else {
            callback(null, common.translateVm(items[0], true));
        }
    });
};



/*
 * Gets a list of vms from UFDS. When no vms are found the second
 * argument to the callback will have an empty array
 */
Ufds.prototype.listVms = function (params, callback) {
    var baseDn;
    var owner_uuid = params.owner_uuid;
    var uuid = params.uuid;
    var filter = '';

    if (owner_uuid) {
        if (!common.validUUID(owner_uuid))
            return callback(
              new restify.InvalidArgumentError('Owner UUID is not ' +
                'a valid UUID'));

        baseDn = sprintf(USER_FMT, owner_uuid);
    } else {
        baseDn = USERS;
    }

    if (uuid) {
        if (!common.validUUID(uuid))
            return callback(
              new restify.InvalidArgumentError('VM UUID is not a valid UUID'));
        filter += '(uuid=' + uuid + ')';
    } else {
        filter += '(uuid=*)';
    }

    if (params.brand)
        filter += '(brand=' + params.brand + ')';

    if (params.alias)
        filter += '(alias=' + params.alias + ')';

    if (params.state) {
        if (params.state == 'active')
            filter += '(!(state=destroyed))';
        else
            filter += '(state=' + params.state + ')';
    }

    if (params.ram)
        filter += '(max_physical_memory=' + params.ram + ')';

    if (params.server_uuid)
        filter += '(server_uuid=' + params.server_uuid + ')';

    if (params.image_uuid)
        filter += '(image_uuid=' + params.image_uuid + ')';


    filter = common.addTagsFilter(params, filter);

    var options = {
        scope: 'sub',
        filter: '(&(objectclass=vm)' + filter + ')'
    };

    return this.search(baseDn, options, function (err, items) {
        if (err) {
            callback(err, null);
            return;
        }

        var vms = [];

        for (var i = 0; i < items.length; i++) {
            vms.push(common.translateVm(items[i], true));
        }

        callback(null, vms);
    });
};



/*
 * Marks a vm as destroyed
 */
Ufds.prototype.markAsDestroyed = function (cache, vm, callback) {
    if (!this.connected) {
        self.log.error('Cannot mark as destroyed without connection to UFDS');
        return;
    }

    var self = this;

    function cacheMarkAsDestroyed(m) {
        var prms = {
            state: 'destroyed',
            zone_state: 'destroyed',
            destroyed: new Date()
        };

        m.state = prms.state;
        m.zone_state = prms.zone_state;
        m.destroyed = prms.destroyed;
        cache.setVm(m.uuid, m, function (err) {
            if (err) {
                self.log.error('Could not mark VM as destroyed in cache', err);
            }
        });

        return prms;
    }

    var params = cacheMarkAsDestroyed(vm);

    self.updateVm(vm, params, function (err) {
        if (err) {
            callback(err);
        } else {
            callback(null);
        }
    });
};



/*
 * Adds metadata to a vm on UFDS. mdataKey can be 'customer_metadata',
 * 'internal_metadata' or 'tags'.
 */
Ufds.prototype.addMetadata = function (vm, mdataKey, params, callback) {
    var mdata = common.clone(vm[mdataKey]);
    var numMetadata = 0;

    Object.keys(params).forEach(function (key) {
        if (key != 'uuid' && key != 'owner_uuid' && key != 'metadata') {
            mdata[key] = params[key];
            numMetadata++;
        }
    });

    if (numMetadata == 0) {
        return callback(
          new restify.InvalidArgumentError('At least one ' + mdataKey +
          ' key must be provided'),
          null);
    }

    var updateParams = {};
    updateParams[mdataKey] = JSON.stringify(mdata);

    return this.updateVm(vm, updateParams, function (err) {
        if (err) {
            callback(err);
            return;
        }

        callback(null, mdata);
    });
};



/*
 * Deletes vm metadata from UFDS
 */
Ufds.prototype.deleteMetadata = function (vm, mdataKey, key, callback) {
    var mdata = common.clone(vm[mdataKey]);

    delete mdata[key];

    var updateParams = {};
    updateParams[mdataKey] = JSON.stringify(mdata);

    this.updateVm(vm, updateParams, function (err) {
        if (err) {
            callback(err);
            return;
        }

        callback(null);
    });
};



/*
 * Deletes all metadata for a vm on UFDS
 */
Ufds.prototype.deleteMetadata = function (vm, mdataKey, callback) {
    var updateParams = {};
    updateParams[mdataKey] = [];

    this.updateVm(vm, updateParams, function (err) {
        if (err) {
            callback(err);
            return;
        }

        callback(null);
    });
};




module.exports = Ufds;
