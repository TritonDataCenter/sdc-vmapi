...
Ufds.prototype.getMachine = function (params, callback) {
    ...
    if (!common.validUUID(uuid))
        return callback(
          new restify.InvalidArgumentError('Machine UUID is not a valid UUID'));
...
/*
 * Marks a machine as destroyed
 */
Ufds.prototype.markAsDestroyed = function (cache, machine, callback) {
    var self = this;
    ...
    var params = cacheMarkAsDestroyed(machine);

    self.updateMachine(machine, params, function (err) {
        if (err)
            return callback(err);
