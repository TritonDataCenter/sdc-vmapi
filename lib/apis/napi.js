...
Napi.prototype.addNics = function (vm) {
    ...
    function getAddNic(nic) {
        function onAddNic(aerr) {
            if (aerr) {
                self.log.info('Could not add NIC ' + nic.mac +
                    ' for VM ' + vm.uuid);
                self.log.error(aerr);
            } else {
                self.log.info('NIC ' + nic.mac +
                    ' added for VM ' + vm.uuid);
            }
        }

        function onGetNic(nerr, aNic) {
            if (nerr) {
                self.log.error(nerr);
            } else {
                if (!aNic) {
                    var allParams = common.simpleMerge(nic, postParams);
                    self.addNic(allParams, onAddNic);
                } else {
...
Napi.prototype.deleteNics = function (vm) {
    ...
    for (var i = 0; i < vm.nics.length; i++) {
        ...
        function onDeleteNic(err) {
            if (err) {
                self.log.info('Could not delete NIC ' + nic.mac +
                    ' for VM ' + vm.uuid);
                self.log.error(err);
            } else {
                self.log.info('NIC ' + nic.mac + ' for VM ' + vm.uuid +
                    ' deleted from NAPI');
            }
        }

        self.deleteNic(nic.mac, onDeleteNic);
    }
};
