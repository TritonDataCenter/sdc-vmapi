...
Moray.prototype._vmsListParams = function _vmsListParams(params, cb) {
    ...
    var options = this._addFilterOptions(params);
    return cb(null, filter, options);
};
...
Moray.prototype.listVms = function (params, cb) {
    var self = this;
    return self._vmsListParams(params, function (err, filter, options) {
        if (err) {
            return cb(err);
        }
...
Moray.prototype.countVms = function countVms(params, cb) {
    ...
    return self._vmsListParams(params, function (err, filter, options) {
        ...
        req.on('record', function (r) {
            // If we are limiting the search then LIMIT is what we want
            if (options.limit) {
                count = options.limit;
            } else if (r && r.count) {
                count = r.count;
