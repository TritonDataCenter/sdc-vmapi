...
Wfapi.prototype.createAddNicsJob = function createAddNicsJob(req, cb) {
    ...
    if (req.vm.internal_metadata !== undefined &&
        ...
        if (req.vm.internal_metadata.set_resolvers === false) {
            params.wantResolvers = false;
        }

        if (req.vm.internal_metadata.custom_resolvers) {
            params.wantResolvers = false;
        }
    }
...
Wfapi.prototype.createMigrateJob = function _createMigrateJobFn(req, cb) {
    ...
    /*
     * Dispatch.  The migration record's `mechanism` field (frozen on
     * begin) selects the workflow variant:
     *   mechanism='cold' -> migrate-<action>       (historical path)
     *   mechanism='live' -> migrate-<action>-live  (bhyve live migration)
     *
     * Only the data-plane phases (begin / sync / switch) have a
     * live-specific workflow; everything else (estimate, finalize,
     * abort, rollback, pause) is pure VMAPI/NAPI record cleanup and
     * reuses the mechanism-agnostic base workflow for both paths.
     * That avoids having to ship duplicate migrate-abort-live /
     * migrate-rollback-live workflows that would differ from cold
     * only in having "-live" in their name.
     */
    var record = req.migrationTask.record;
    var mechanism = (record && record.mechanism) || 'cold';
    var LIVE_SPECIFIC_ACTIONS = ['begin', 'sync', 'switch'];
    var workflowTask = 'migrate-' + action;
    if (mechanism === 'live' &&
            LIVE_SPECIFIC_ACTIONS.indexOf(action) !== -1) {
        workflowTask = 'migrate-' + action + '-live';
    }
