...
VMAPI.prototype._initApis = function () {
    ...
    this.server.on('after', function (req, res, route, error) {
        restify.auditLogger({log: req.log, body: true})(req, res, route, error);
    });
    ...
    config.amqp.log = log.child({ component: 'heartbeater' }, true);
    ...
    config.redis.log = log.child({ component: 'redis' }, true);
...
VMAPI.prototype.init = function () {
    var log = this.log;
    var self = this;
    self.listen(onListen);

    function onListen() {

        var functions = [
            self.moray.connect.bind(self.moray),
            self.cache.connect.bind(self.cache),
            self.heartbeater.connect.bind(self.heartbeater),
            self.heartbeater.listen.bind(self.heartbeater, self),
            self.wfapi.connect.bind(self.wfapi),
            function checkDataLoaded(cb) {
                self.moray.countVms({}, function (err, count) {
                    if (err) {
                        log.error(err, 'Could not check for VM data on moray');
                    } else if (count === 0) {
                        self.status = self.statuses.POPULATING;
                    } else {
