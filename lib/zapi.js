...
ZAPI.prototype.onHeartbeats = function (serverUuid, hbs) {
    ...
    function cnapiThenUfds(uuid, status, lastModified) {
        self.cnapi.getMachine(serverUuid, uuid, function (err, machine) {
            if (err) {
                ...
                var cacheObject = {
                    ...
                    machine: common.machineToUfds(machine)
                };
                self.cache.set(uuid, cacheObject);
                self.ufds.addReplaceMachine(machine);
            }
        });
    }
    ...
    for (var i = 0; i < hbs.length; i++) {
        ...
        if (uuid != 'global') {
            ...
            if (!oldMachine) {
                cnapiThenUfds(uuid, status, lastModified);
            }

            if (oldMachine &&
                (oldMachine.status != status ||
                    oldMachine.lastModified < lastModified)) {
                cnapiThenUfds(uuid, status, lastModified);
            }
        }
    }
