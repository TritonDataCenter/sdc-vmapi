...
ZAPI.prototype.onHeartbeats = function (serverUuid, hbs, missing) {
        ...
        if (uuid != 'global') {
            ...
            if (oldMachine &&
                (oldMachine.zone_state != zone_state ||
                    oldMachine.last_modified < lastModified)) {
                cnapiThenUfds(uuid);
            }
        }
    }

    function markDestroyed(machine) {
        var params = {
            state: 'destroyed',
        ...
        machine.state = params.state;
        machine.zone_state = params.zone_state;
        machine.destroyed = params.destroyed;
        self.cache.set(machine.uuid, machine);
    ...
    for (var i = 0; i < missing.length; i++) {
        ...
        if (oldMachine) {
            var params = markDestroyed(oldMachine);
            self.ufds.updateMachine(oldMachine, params, function (err) {
                if (err) {
