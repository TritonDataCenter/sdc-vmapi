...
Ufds.prototype.listMachines = function(params, callback) {
  ...
  if (owner_uuid) {
    if (!common.validUUID(owner_uuid))
      return callback(new restify.ConflictError('Owner UUID is not a valid UUID'), null);
...
Ufds.prototype.addTags = function(machine, params, callback) {
  ...
  if (!keys.length)
    return callback(new restify.InvalidArgumentError('At least one tag must be provided'), null);
