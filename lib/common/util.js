...
function pollJob(opts, callback) {
    ...
    function _getErrorFromJob(job, errorMessage) {
        var result = job && job.chain_results && job.chain_results.pop() || {};

        if (result.error) {
            var m = sprintf('job %s (%s) failed: %s: %s',
                job.name, job_uuid, result.name, result.error);
            m = result.error.message ? result.error.message : m;

            return vmapierrors.wfapiWrap({
                error: result.error,
                message: m
            });
        }

        var errorPrefix = sprintf('Job %s (%s) ', job.name, job_uuid);

        if (job.execution === 'canceled') {
            return new Error(errorPrefix + 'was canceled');
        }

        if (job.execution === 'failed') {
            return new Error(errorPrefix + 'failed');
        }

        if (errorMessage) {
            return new Error(errorPrefix + errorMessage);
        }

        return new Error(errorPrefix + sprintf('had an unexpected state "%s"',
            job.execution));
    }
    ...
    function notificationHandler(event) {
        ...
        if (jobCompletedStates.indexOf(event.execution) !== -1) {
            ...
            if (event.execution !== 'succeeded') {
                cb(_getErrorFromJob(job), job);
                return;
            }
            // Success.
            cb(null, job);
        ...
        wfapi.getJob(job_uuid, function (err, job) {
            ...
            log.debug({ job: job }, 'polling job %s (attempt %d)',
                      ...
                      if (job && job.execution === 'succeeded') {
                          ...
                          return (cb(_getErrorFromJob(job), job));
                          ...
                          return (cb(_getErrorFromJob(job,
 ...
 */
exports.waitForJob = function waitForJob(opts, cb) {
    ...
    opts.log.info('waiting for job %s', opts.job_uuid);
