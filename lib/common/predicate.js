...
function ldapPrintRel(pred, key) {
	...
	if (typeof (field) !== 'string') {
		return printKeyValue(key, field, pred[key][1]);
	}

	switch (field) {
		case 'ram':
			return printKeyValue(key, 'max_physical_memory', pred[key][1]);
			break;
		default:
			if (field.match(/tag\.(.*)/)) {
				return printTags(field, pred[key][1])
			}
			return printKeyValue(key, field, pred[key][1]);
	}
}
 ...
 *
 * Input:
 *  - pred: A predicate that has already been validated by predValidate
 *
 * Output:
 *  - Returns the string representation of the specified predicate.
 */
