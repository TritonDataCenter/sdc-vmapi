...
function validOwner(vm, params) {
    ...
    if (owner_uuid) {
        if (!validUUID(owner_uuid)) {
            /*JSSTYLED*/
            throw new restify.InvalidArgumentError('\'owner_uuid\' is not a valid UUID');
        }
...
function createMetadataObject(vm, mdataKey, params, obj) {
    ...
    if (typeof (obj) === 'object') {
        ...
        try {
            ...
            /*JSSTYLED*/
    		throw new restify.InvalidArgumentError('\'%s\' is not valid \'' + mdataKey + '\'', obj);
        }
    } else {
        /*JSSTYLED*/
        throw new restify.InvalidArgumentError('\'' + mdataKey + '\' ' + obj + ' is invalid');
    }
