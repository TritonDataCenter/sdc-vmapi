...
exports.validateUpdateVmParams = function (vmapi, vm, obj, callback) {
    ...
    function validateResize(next) {
        // Checks if VM is being downsized. Downsize is not supported at the
        // moment. This check is done once we have loaded the new package so
        // that calling params[field] has all the values we need
        function isDownsize() {
            var newRam = params.ram || params.max_physical_memory;

            return (newRam && newRam < vm.ram ||
                params.quota && params.quota < vm.quota);
        }

        if (isDownsize()) {
            errs.push(errors.invalidParamErr(
                'billing_id',
                'Cannot downsize a VM to a smaller Package'));
            return next();
        }
        ...
        function onImage(err, image) {
            ...
            // Only check max values because downsizing is not supported
