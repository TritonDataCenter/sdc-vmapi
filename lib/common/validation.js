 ...
 */
exports.validateVm = function (vmapi, params, callback) {
    return validateParams(vmapi, params, callback);
};
 ...
 * Validates a vm representation given a set of request parameters
 */
function validateParams(vmapi, params, callback) {
    var message;
    var errs = [];
    ...
    // validate and cast to numbers. value is passed to validNumber as gezero
    var numKeys = {
        'ram': false,
        'vcpus': false,
        'overprovision_cpu': false,
        'overprovision_memory': false,
        'cpu_cap': true,
        'cpu_shares': true,
        'max_lwps': true,
        ...
        'quota': true,
        'tmpfs': true,
        'zfs_io_priority': true
    };

    for (var key in numKeys) {
        if (params[key] !== undefined) {
            if (validNumber(params[key], numKeys[key])) {
                params[key] = Number(params[key]);
            } else {
                errs.push(errors.invalidParamErr(key, 'Not a valid number'));
            }
        }
    }
    ...
    // maintain_resolvers
    if (params.hasOwnProperty('maintain_resolvers') &&
        (typeof (params.maintain_resolvers) !== 'boolean')) {
        errs.push(errors.invalidParamErr('maintain_resolvers'));
    }
    ...
    if (params['indestructible_zoneroot'] !== undefined &&
        typeof (params['indestructible_zoneroot']) !== 'boolean') {
        errs.push(errors.invalidParamErr('indestructible_zoneroot'));
    }

    // indestructible_delegated
    if (params['indestructible_delegated'] !== undefined &&
        typeof (params['indestructible_delegated']) !== 'boolean') {
        errs.push(errors.invalidParamErr('indestructible_delegated'));
    }
 ...
 * - indestructible_delegated
 */
exports.validateUpdate = function (vmapi, vm, obj, callback) {
    var errs = [];
    var params = {};
    ...
    for (var key in numKeys) {
        if (obj[key] !== undefined) {
            if (validNumber(obj[key], numKeys[key])) {
                params[key] = Number(obj[key]);
            } else {
                errs.push(errors.invalidParamErr(key, 'Not a valid number'));
            }
        }
    ...
    if (obj.maintain_resolvers !== undefined) {
        if (typeof (obj.maintain_resolvers) === 'boolean') {
            params.maintain_resolvers = obj.maintain_resolvers;
        } else {
            errs.push(errors.invalidParamErr('maintain_resolvers'));
        }
    }
    ...
    if (obj['indestructible_zoneroot'] !== undefined) {
        if (typeof (obj['indestructible_zoneroot']) === 'boolean') {
            params['indestructible_zoneroot'] = obj['indestructible_zoneroot'];
        } else {
            errs.push(errors.invalidParamErr('indestructible_zoneroot'));
        }
    }

    if (obj['indestructible_delegated'] !== undefined) {
        if (typeof (obj['indestructible_delegated']) === 'boolean') {
            params['indestructible_delegated'] =
                obj['indestructible_delegated'];
        } else {
            errs.push(errors.invalidParamErr('indestructible_delegated'));
        }
    }
