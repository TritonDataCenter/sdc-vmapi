...
function validNetworks(object) {
    ...
    for (var i = 0; i < array.length; i++) {
        ...
        if (!uuid || !UUID_RE.test(uuid)) {
            throw new Error(format('Invalid uuid %s', uuid));
        }
        networks.push(obj);
    }
...
function validateBrandParams(params, errs) {
    if (params.brand === 'kvm') {
        ...
        } else if (!Array.isArray(params.disks) || (params.disks.length < 1)) {
...
function validateParams(moray, params, callback) {
    ...
    } else if (params.ram && !validNumber(params.ram)) {
    ...
    if (params.networks) {
        try {
            params.networks = validNetworks(params.networks);
        } catch (e) {
...
exports.validateUpdate = function (moray, vm, obj, callback) {
    ...
    Object.keys(properties).forEach(function (prop) {
        // Only allow >= 0 for quota
        gezero = (prop == 'quota' ? true : false);
