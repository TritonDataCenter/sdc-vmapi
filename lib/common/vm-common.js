...
function tagFormatToObject(string) {
    ...
    if (array.length < 3) {
        throw new TypeError('Incorrect tags string format');
    }
 ...
 *     input: 'tag.role' ]
 */
exports.addTagsFilter = function (params, conditions) {
    var filters = [];
    var string = (conditions === 0 ? ' WHERE ' : ' AND ');

    Object.keys(params).forEach(function (key) {
        var matches = key.match(/tag\.(.*)/);
        if (matches) {
            var tag = matches[1];
            filters.push('tags LIKE \'%-' + tag + '=' + params[key] + '-%\'');
        }
    });
    ...
    if (filters.length === 0) {
        ...
        return string + filters.join(' AND ');
    }
};
