...
function createProvisionPayload(job, cb) {
    ...
    // Remove cpu_cap when null, otherwise vmadm will complain.
    if (vmPayload.cpu_cap === null) {
        delete vmPayload.cpu_cap;
    }

    // Handle delegated datasets for SmartOS and LX zones.
    if (Array.isArray(vmPayload.datasets) && vmPayload.datasets.length > 0) {
        if (['lx', 'joyent', 'joyent-minimal'].indexOf(brand) === -1) {
...
/*
 * Release NICs that a failed begin left behind in NAPI.
 *
 * When provisionVm fails midway through machine_create, cn-agent has
 * often already reserved MAC/IP pairs in NAPI before vmadm bailed.
 * Those reservations (belongs_to_uuid = target_vm_uuid, cn_uuid =
 * target CN, state != 'running') persist indefinitely and poison the
 * next begin: the VMAPI payload builder reads VMAPI's stored VM
 * record, sees the extra ghost NIC carried over from the old failed
 * attempt, and the destination bhyve ends up with a mismatched PCI
 * layout (observed as an assertion failure in pci_emul.c).
 *
 * Filter rules (critical for safety):
 *   1. Must belong to this VM's target_vm_uuid.
 *   2. cn_uuid must match the target CN — we must NEVER delete the
 *      source's live NIC if it shares the uuid.
 *   3. Skip NICs in state 'running' — those are real, in-use by a
 *      running VM on the target (which in live-migration after a
 *      successful cutover can legitimately hold them).
 *
 * Best-effort: listNics / deleteNic failures log and continue.  This
 * runs on the workflow's onerror path, so a failure to clean up
 * doesn't re-fail an already-failed job.
 */
function cleanupTargetNics(job, cb) {
    var record = job.params.migrationTask && job.params.migrationTask.record;

    if (!record || !record.target_vm_uuid || !record.target_server_uuid) {
        cb(null, 'OK - no target_{vm,server}_uuid on record, skipping');
        return;
    }
    ...
    napi.listNics({belongs_to_uuid: record.target_vm_uuid},
            function _onListCb(listErr, nics) {
        if (listErr) {
            job.log.warn({err: listErr},
                'cleanupTargetNics: listNics failed, skipping');
            cb(null, 'OK - listNics failed (non-fatal): ' + listErr.message);
            return;
        }

        var ghosts = (nics || []).filter(function _isGhost(nic) {
            return nic.cn_uuid === record.target_server_uuid &&
                nic.state !== 'running';
        });

        if (ghosts.length === 0) {
            cb(null, 'OK - no ghost NICs to clean up (of ' +
                (nics || []).length + ' total)');
            return;
        }
        ...
        job.log.info({
            count: ghosts.length,
            total: nics.length,
            target_vm_uuid: record.target_vm_uuid,
            target_server_uuid: record.target_server_uuid
        }, 'cleanupTargetNics: releasing ghost NICs from failed begin');

        var remaining = ghosts.length;
        var deleted = 0;
        ghosts.forEach(function _del(nic) {
            napi.deleteNic(nic.mac, function _onDeleteCb(delErr) {
                if (!delErr ||
                        (delErr.body && delErr.body.code === 'ResourceNotFound')) {
                    deleted++;
                } else {
                    job.log.warn({err: delErr, mac: nic.mac},
                        'cleanupTargetNics: deleteNic failed');
                }
                remaining--;
                if (remaining === 0) {
                    cb(null, 'OK - released ' + deleted + '/' +
                        ghosts.length + ' ghost NICs');
                }
            });
        });
    });
...
module.exports = {
    tasks: {
        ...
        cleanupTargetNics: {
            name: 'migration.cleanupTargetNics',
            timeout: 120,
            retry: 1,
            body: cleanupTargetNics,
            modules: {
                sdcClients: 'sdc-clients'
            }
        },
