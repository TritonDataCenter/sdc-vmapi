...
function generatePasswords(job, cb) {
    ...
    for (var i = 0; i < users.length; i++) {
        user = users[i].name;
        if (job.params['internal_metadata'][user + '_pw']  === undefined) {
            job.params['internal_metadata'][user + '_pw'] = randomPassword(10);
        }
    }
...
function getAllocation(job, cb) {
    ...
    var payload = {
        ...
        vm: { ram:        vm.ram,
              ...
              traits:     vm.traits,
              overprovision_cpu:  vm.overprovision_cpu,
              overprovision_ram:  vm.overprovision_memory,
              overprovision_disk: vm.overprovision_storage,
              overprovision_io:   vm.overprovision_io,
              overprovision_net:  vm.overprovision_network },
