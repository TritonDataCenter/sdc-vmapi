    ...
    return imgapi.get(path, function (err, req, res, image) {
        if (err) {
            ...
            if (image.state !== 'active' || image.disabled !== false) {
                return cb('Can only provision with active images');
            }
            job.image = image;
 ...
 * onto which you want to provision.
 */
function getServers(job, cb) {
    ...
    function napiGetUuids(next) {
        ...
        if (job.params['server_uuid']) {
            params['belongs_to_uuid'] = job.params['server_uuid'];
        }

        function addNicData(napiFn, cb2) {
            napi[napiFn].call(napi, params, function (err, nics) {
                if (err) {
                    cb2(err);
                } else {
                    for (var j = 0; j < nics.length; j++) {
                        ...
                        // Might be the case that we want 2 nics on the
                        // same network
        ...
        async.forEach(['listNics', 'listAggrs'], addNicData, function (err) {
            if (serverUuids.length) {
                job.log.info({ serverUuids: serverUuids },
                    'Server UUIDs retrieved');
                next();
            } else {
                next(new Error('No servers found that satisfy required ' +
                    ' NIC tags'));
            }
        });
    },
            ...
            if (err) {
                ...
                if (Array.isArray(servers) && servers.length) {
                    servers.forEach(function (s) {
                        job.serversByUuid[s.uuid] = s;
                    });

                    job.servers = servers;
                    job.log.debug(Object.keys(job.serversByUuid),
...
function getAllocation(job, cb) {
    if (job.params['server_uuid']) {
        job.serverSysinfo =
            job.serversByUuid[job.params['server_uuid']].sysinfo;
    ...
    return dapi.post('/allocation', payload, function (err, req, res, body) {
        if (err) {
            ...
            job.serverSysinfo = job.serversByUuid[body.server.uuid].sysinfo;
            cleanup();
...
function preparePayload(job, cb) {
    ...
    var keys = [ 'alias', 'autoboot', 'billing_id', 'brand', 'cpu_cap',
        ...
        'indestructible_delegated', 'internal_metadata', 'limit_priv',
...
function ensureImage(job, cb) {
    var imageUuid;
    /**
     * If server 6.5 we can skip the next step, as provisioner will zfs receive
     * the dataset into the zpool itself.
     */
    if (job.serverSysinfo['SDC Version'] === '7.0') {
        var commonHeaders = { 'x-request-id': job.params['x-request-id'] };
        var cnapi = new sdcClients.CNAPI({
            url: cnapiUrl, headers: commonHeaders });
        ...
        cb(null, 'Compute node not running SDC7, will import image itself');
    }
}
