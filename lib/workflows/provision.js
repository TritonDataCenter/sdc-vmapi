...
function generatePasswords(job, cb) {
    ...
    if (job.params['customer_metadata'] === undefined) {
        job.params['customer_metadata'] = {};
    }
    ...
    for (var i = 0; i < users.length; i++) {
        user = users[i].name;
        if (job.params['customer_metadata'][user + '_pw']  === undefined) {
            job.params['customer_metadata'][user + '_pw'] = randomPassword(10);
        }
    }
