...
var fwCommon = require('wf-shared').fwapi;
...
function getImage(job, cb) {
    // Make sure to not break future manta beta deployments for now
    if (job.params.brand !== 'kvm') {
        return cb(null, 'Skipping imgapi.get_image when not KVM');
    }
...
function getServers(job, cb) {
    ...
    var params = { uuids: job.serverUuids.join(',') };

    return cnapi.listServers(function (err, servers) {
        if (err) {
            return cb(err);
        } else {
...
function provision(job, cb) {
    ...
    function preparePayload(params) {
        ...
        if (params['brand'] === 'kvm') {
            ...
            // Remove
            payload['image_uuid'] = job.image.uuid;
