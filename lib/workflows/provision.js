    ...
    return imgapi.get(path, function (err, req, res, image) {
        if (err) {
            if (job.params.brand !== 'kvm') {
                return cb(null, 'Could not get image, but ignoring for now ' +
                    'to not break manta-beta: ' + err);
            }
            return cb(err);
        } else {
            job.image = image;
...
function generatePasswords(job, cb) {
    ...
    if (cm === undefined) {
        ...
        if (cm.credentials !== undefined &&
            typeof (cm.credentials) === 'string') {
            try {
...
function getAllocation(job, cb) {
    ...
    var payload = {
        ...
        vm: { ram:        vm.ram,
              ...
              traits:     vm.traits,
              overprovision_ratio: vm.overprovision_ratio },
...
function provision(job, cb) {
    ...
    function preparePayload(params) {
        ...
        var keys = [ 'alias', 'autoboot', 'billing_id', 'brand', 'cpu_cap',
            ...
            'max_locked_memory', 'max_lwps', 'max_physical_memory', 'max_swap',
            'nics', 'owner_uuid', 'package_name', 'package_version', 'quota',
            'ram', 'resolvers', 'vcpus', 'zfs_data_compression',
            'zfs_io_priority', 'tags'
        ];
        ...
        // Find resolvers in the primary NIC
        for (i = 0; i <  params.nics.length; i++) {
            nic = params.nics[i];

            if (nic.primary === true && nic["resolvers"] !== undefined) {
                payload["resolvers"] = nic["resolvers"];
            }
        }
        ...
        if (params['brand'] === 'kvm') {
            ...
            if (params['disk_driver'] === undefined) {
                payload['disk_driver'] = job.image['disk_driver'];
            }

            if (params['cpu_type'] === undefined) {
                payload['cpu_type'] = job.image['cpu_type'];
            }

            if (job.image['nic_driver'] !== undefined) {
                payload['nic_driver'] = job.image['nic_driver'];
            }
        } else {
