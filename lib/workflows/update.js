...
function validateUpdateParameters(job, cb) {
    ...
    vmapi.getVm({ uuid: job.params['vm_uuid'] }, function onVmapi(err, vm) {
        if (err) {
            return cb(err);
        }
        var ram = job.params['vm'] || job.params['max_physical_memory'];
        var quota = job.params['quota'];

        // First check this is not a KVM VM and then use ram and quota to
        // determine when a VM is being resized
        if (vm.brand === 'kvm') {
            return cb('Cannot resize a KVM VM');
        } else if (ram !== undefined && ram < vm.ram) {
            return cb('Cannot downsize RAM for a VM');
        } else if (quota !== undefined && quota < vm.quota) {
            return cb('Cannot downsize quota for a VM');
        }
