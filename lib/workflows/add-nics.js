...
function checkServerNicTags(job, cb) {
    ...
    cnapi.getServer(job.params.server_uuid, function (err, server) {
        ...
        var serverNicTags = mapNics(server.sysinfo['Network Interfaces']);
        for (var i = 0; i < job.nicTags.length; i++) {
            if (serverNicTags.indexOf(job.nicTags[i]) === -1) {
                var msg = 'Server does not have NIC tag: ' + job.nicTags[i];
                return cb(new Error(msg));
            }
        }

        return cb(null, 'Server has all the required NIC tags');
    });
}
...
var workflow = module.exports = {
    ...
    chain: [ {
        ...
        name: 'cnapi.check_server_nic_tags',
        timeout: 10,
        retry: 1,
