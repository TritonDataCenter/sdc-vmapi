...
function getPackage(job, cb) {
    ...
    if (payload.billing_id === undefined ||
    ...
    PAPI.get(payload.billing_id, {}, function (err, pkg) {
        if (err) {
        ...
        packageFields.forEach(function (field) {
            if (payload[field] === undefined && pkg[field] !== undefined) {
                if (field === 'quota') {
                    if (payload.brand === 'kvm') {
                        payload.quota = 10;
                    } else {
                        payload.quota = Number(pkg.quota) / 1024;
                    }
                } else {
                   payload[field] = Number(pkg[field]);
                }
            }
        });
        ...
        if (pkgRam !== undefined) {
            if (payload.max_physical_memory === undefined) {
                payload.max_physical_memory = Number(pkgRam);
            } else if (payload.ram === undefined) {
                payload.ram = Number(pkgRam);
            }
        }
        pkgRam = payload.max_physical_memory || payload.ram;

        if (payload.cpu_shares === undefined) {
            if (pkg.fss !== undefined) {
                payload.cpu_shares = Math.floor(Number(pkg.fss));
                if (payload.cpu_shares < 1) {
                    payload.cpu_shares = 1;
                }
            } else {
                // Last resort default cpu_shares
                if (pkgRam > 128) {
                    payload.cpu_shares = Math.floor(pkgRam / 128);
                } else {
                    payload.cpu_shares = 1;
                }
            }
