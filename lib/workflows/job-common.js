...
function deleteCustomerVm(job, cb) {
    ...
    if (job.markAsFailedOnError === false) {
        return cb(null, 'markAsFailedOnError was set to false, won\'t ' +
            'delete customer vmusage on UFDS');
    }
        ...
        return UFDS.del(dn, function (err) {
            if (err) {
                if (err.name === 'ResourceNotFoundError') {
                    return cb(null, 'VM usage doesn\'t exist on UFDS');
                } else {
                    job.log.info(err, 'Error deleting VM usage from UFDS');
                    return cb('Error deleting VM usage from UFDS. See /info');
                }
            }
    ...
    function asyncProvisionNics(currentNics) {
        async.mapSeries(networks, function (network, next) {
            ...
            var netNics = currentNics.filter(function (nic) {
                return (nic.network_uuid && nic.network_uuid === network.uuid);
            });
...
/*
 * Gets the package from PAPI only if billing_id is passed along the params.
 * Throws an error if the package doesn't exist
 */
function getPackage(job, cb) {
    if (job.params['billing_id'] === undefined ||
        job.params['billing_id'] === '00000000-0000-0000-0000-000000000000') {
    ...
    var PAPI = sdcClients.PAPI({ url: papiUrl, log: job.log });

    PAPI.get(job.params['billing_id'], {}, function (err, pkg) {
        if (err) {
            return cb(err, 'Could not get package for provision');
        }
        job.log.info({ 'package': pkg });
        job['package'] = pkg;

        // Still allow for manually overriding package params from original
        // provision payload
        packageFields.forEach(function (field) {
            if (job.params[field] === undefined && pkg[field] !== undefined) {
                if (field === 'quota') {
                    if (job.params.brand === 'kvm') {
                        job.params.quota = 10;
                    } else {
                        job.params.quota = Number(pkg.quota) / 1024;
                    }
                } else {
                   job.params[field] = Number(pkg[field]);
                }
            }
        });
        ...
        if (pkgRam !== undefined) {
            if (job.params['max_physical_memory'] === undefined) {
                job.params['max_physical_memory'] = Number(pkgRam);
            } else if (job.params['ram'] === undefined) {
                job.params['ram'] = Number(pkgRam);
            }
        }
        pkgRam = job.params['max_physical_memory'] || job.params['ram'];

        if (job.params['cpu_shares'] === undefined) {
            if (pkg.fss !== undefined) {
                job.params['cpu_shares'] =
                    Math.floor(Number(pkg.fss));
                if (job.params['cpu_shares'] < 1) {
                    job.params['cpu_shares'] = 1;
                }
            } else {
                // Last resort default cpu_shares
                if (pkgRam > 128) {
                    job.params['cpu_shares'] = Math.floor(pkgRam / 128);
                } else {
                    job.params['cpu_shares'] = 1;
                }
            }
        ...
        return cb(null, 'Package for provision found on PAPI');
    });
}
...
function acquireVMTicket(job, cb) {
    ...
    var newTicket = {
        ...
        expires_at: (new Date(
            new Date().valueOf() + 600 * 1000).toISOString()),
        action: job.action,
...
function waitOnVMTicket(job, cb) {
    ...
    if (ticket.status === 'active') {
        ...
        cnapi.waitlistTicketWait(job.ticket.uuid, function (err) {
            if (err) {
                cb(err);
                return;
            }
        ...
        cb(new Error('Unexpected ticket status'));
    }
}
