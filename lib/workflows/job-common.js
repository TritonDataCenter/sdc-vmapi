...
/*
 * Gets the package from UFDS only if billing_id is passed along the params.
...
function getPackage(job, cb) {
    ...
    UFDS.on('ready', function () {
        ...
        var filter = '(&(objectclass=sdcpackage)(uuid=' +
            job.params['billing_id'] + '))';
        var options = {
            scope: 'sub',
            filter: filter
        };
        ...
        UFDS.search(base, options, function (err, packages) {
            ...
            packageFields.forEach(function (field) {
                if (job.params[field] === undefined &&
                    packages[0][field] !== undefined) {
                    if (field === 'quota') {
                        if (job.params.brand === 'kvm') {
                            job.params.quota = 10;
                        } else {
                            job.params.quota = Number(packages[0].quota) / 1024;
                        }
                    } else {
                       job.params[field] = Number(packages[0][field]);
                    }
                }
            });

            // Special cases
            var pkgRam = packages[0]['max_physical_memory'];
            if (pkgRam !== undefined) {
            ...
            if (job.params['cpu_shares'] === undefined) {
                if (packages[0].fss !== undefined) {
                    job.params['cpu_shares'] =
                        Math.floor(Number(packages[0].fss));
