...
function pollTask(job, cb) {
    ...
    // As soon was we reach this point, we don't want to clean up NICs when a
    // provision fails
    job.cleanupNicsOnError = false;
...
function deleteCustomerVm(job, cb) {
    ...
    if (job.cleanupNicsOnError === false) {
        return cb(null, 'cleanupNicsOnError was set to false');
    }
        ...
        UFDS.search(base, options, function (err, packages) {
            ...
            if (job.params['cpu_shares'] === undefined) {
                if (packages[0].fss !== undefined) {
                    job.params['cpu_shares'] = Math.floor(Number(packages[0].fss));
