...
Cache.prototype.setVmState = function(uuid, newState, server, callback) {
    var self = this;

    var machineState = (newState == 'running' || newState == 'shutting_down')
                     ? 'running'
                     : 'stopped';
    ...
    function onGetVm(readErr, cached) {
        ...
        cached['zone_state'] = newState;

        self.setVm(uuid, common.translateVm(cached, true), function (writeErr) {
            if (writeErr)
    ...
    self.setState(uuid, newState, onSaveState);
};
