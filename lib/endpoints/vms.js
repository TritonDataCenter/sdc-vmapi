    ...
    if (m.server_uuid && req.params.sync && req.params.sync == 'true') {
        req.cnapi.getVm(m.server_uuid, m.uuid, function (err, vm) {
            ...
            if (vm) {
                ...
                res.send(200, newVm);
                req.cache.setVm(newVm.uuid, newVm, function (cacheErr) {
                    req.log.error('Could not update VM on cache');
                });
            } else {
                markAsDestroyed(req);
                res.send(200, req.vm);
...
function createVm(req, res, next) {
    ...
    common.validateVm(req.ufds, req.params, function(error) {
        ...
        function (err, vmuuid, juuid) {
            ...
            req.cache.setVm(vm.uuid, vm, function (cacheErr) {
                req.log.error('Could not set new VM on cache');
            });
