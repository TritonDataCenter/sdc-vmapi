...
function listVms(req, res, next) {
    ...
    req.moray.countVms(req.params, function (err, count) {
        if (err) {
            return next(err);
        }

        res.header('x-joyent-resource-count', count);
        req.moray.listVms(req.params, function (err, vms) {
            if (err) {
                return next(err);
            }
...
function markAsDestroyed(req, res, next) {
    if (req.params.sync && req.params.sync == 'true') {
        ...
        req.moray.markAsDestroyed(req.cache, req.vm, onCacheMark);
    }
...
function deleteVm(req, res, next) {
    ...
    if (!req.vm['server_uuid']) {
        /*JSSTYLED*/
        next(new errors.UnallocatedVMError('Cannot delete a VM that has not been allocated to a server yet'));
        return;
    }
 ...
 * Snapshots a VM
 */
function snapshotVm(req, res, next) {
    req.log.trace('SnapshotVm start');
 ...
 * Rollbacks a VM
 */
function rollbackVm(req, res, next) {
    req.log.trace('RollbackVm start');

    if (!req.params.name) {
        return next(new restify.MissingParameterError('name is required'));
    }

    req.wfapi.createRollbackJob(req, function (err, juuid) {
        if (err) {
            return next(err);
        }
...
function mount(server, before) {
    ...
    server.post({ path: '/vms/:uuid/snapshot', name: 'SnapshotVm' },
                  before, snapshotVm, streamResponse);

    server.post({ path: '/vms/:uuid/rollback', name: 'RollbackVm' },
                  before, rollbackVm, streamResponse);
}
