...
function changeVm(req, res, next) {
    ...
    common.validateUpdate(req.moray, req.vm, req.params, onValidate);
    ...
    function onValidate(verr, params) {
        ...
        req.wfapi.createUpdateJob(req, params, function (err, juuid) {
            if (err) {
                return next(err);
            }
...
function markAsDestroyed(req, res, next) {
    if (req.params.sync && req.params.sync == 'true') {
        req.napi.deleteNics(req.vm, onNapiDeleted);
