 ...
 */
function getServerVms(req, res, next) {
    req.log.trace('GetServerVms start');

    if (req.app.config.useVmAgent !== true) {
    ...
    var error;
    var server_uuid = req.query.server_uuid;
    ...
    var query = { server_uuid: server_uuid, state: 'active' };
    req.app.moray.listVms(query, function (err, vms) {
        if (err) {
            return next(err);
        }
...
/*
 * Updates a VM record on Moray
 */
function updateVmOnMoray(moray, uuid, vm, cb) {
    moray.putVm(uuid, vm, function (err) {
        if (err) {
            return cb(err);
        }
        return cb();
...
function putVms(req, res, next) {
    ...
    var allUuids = Object.keys(req.params.vms);

    req.log.debug('List of VMs reported by vm-agent for server %s',
        req.query.server_uuid,
        Object.keys(req.params.vms).sort());
    req.log.debug('List of VMs reported by moray for server %s',
        req.query.server_uuid,
        Object.keys(req.vms).sort());

    // For now, cleanup VM nics as well until net-agent takes care of this
    function markVmAsDestroyed(uuid, cb) {
        var vm = req.vms[uuid];
        ...
        req.app.moray.markAsDestroyed(vm, function (err) {
            ...
            req.app.napi.deleteNics(vm, function (napiErr) {
                if (napiErr) {
                    req.log.error(napiErr, 'Error deleting NICs for %', uuid);
                }
                return cb();
            });
        });
    ...
    // If there is a VM in moray that doesn't exist in the server then we add
    // it to allUuids and then we check if it exists in req.params.vms. If it
    // isn't there we mark it was destroyed.
    Object.keys(req.vms).forEach(function (uuid) {
        if (allUuids.indexOf(uuid) === -1) {
            allUuids.push(uuid);
        }
    });

    async.eachSeries(allUuids, function (uuid, cb) {
        // undefined means server doesn't know about this VM anymore
        if (req.params.vms[uuid] !== undefined) {
            updateVmOnMoray(req.app.moray, uuid, req.params.vms[uuid], cb);
        } else {
            req.log.debug({ vm_uuid: uuid },
                'Marking VM %s as destroyed', uuid);
            markVmAsDestroyed(uuid, cb);
        }
    }, function (err) {
        if (err) {
            return next(err);
...
function putVm(req, res, next) {
    req.log.trace('PutVm start');

    if (req.app.config.useVmAgent !== true) {
        req.log.info('PutVm called and vm-agent feature is not active');
        res.send(200);
        return next();
    }
    ...
    updateVmOnMoray(req.app.moray, req.params.uuid, vm, function (err) {
        if (err) {
            return next(err);
        }
        res.send(200, vm);
