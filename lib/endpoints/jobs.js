 ...
 * GET /jobs/:uuid/wait
 */
function waitJob(req, res, next) {
    req.log.trace('WaitJob start');
    ...
    common.waitForJob(opts, function _onJobEnd(err) {
        if (err) {
            next(err);
            return;
        }
...
function mount(server) {
    ...
    server.get({ path: '/jobs/:job_uuid/wait', name: 'WaitJob' }, waitJob);
