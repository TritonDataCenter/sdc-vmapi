...
function getMachine(req, res, next) {
    ...
    if (m.server_uuid && req.params.sync && req.params.sync == 'true') {
        req.cnapi.getMachine(m.server_uuid, m.uuid, function (err, machine) {
            if (err)
                return next(err);

            if (machine) {
                var newMachine = common.translateMachine(machine, true);
                req.cache.set(newMachine.uuid, newMachine);
                res.send(200, newMachine);
            } else {
                markAsDestroyed(req);
...
/*
 * Helper function for marking a machine as destroyed
 */
