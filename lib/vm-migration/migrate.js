...
/*
 * Migration mechanism — selected on 'begin' and persisted on the record
 * for the life of the migration.  Subsequent actions (sync / switch /
 * rollback / abort) inherit the mechanism from the record, so operators
 * do not have to re-specify it.
 *
 * - 'cold' : stop the source VM, zfs-send, boot the target.  The
 *            historical Triton migration mode.  Works for every brand.
 * - 'live' : pre-copy RAM + transfer bhyve kernel/device state while
 *            the source stays running; pause-and-cutover window is
 *            bounded by the dirty-page convergence of the last pass.
 *            bhyve only, requires the per-CN vmm-migrate-agent on
 *            source and destination CNs.
 */
var VALID_MIGRATION_MECHANISMS = ['cold', 'live'];
var DEFAULT_MIGRATION_MECHANISM = 'cold';
...
function validateMigrationBegin(vm, ctx, callback) {
    ...
    if (ctx.migrationRecord) {
        ...
        // Note that state === 'aborted' does not exist anymore - it is here
        // for backward compatability (old migration records could contain
        // this state).
    ...
    // Cannot migrate a core Triton instance, except we do allow NAT zones.
    if (vm.tags && vm.tags.smartdc_type === 'core' &&
            vm.tags.smartdc_role !== 'nat') {
        callback(new restify.errors.PreconditionFailedError(
            'Cannot migrate a core instance'));
        return;
    }

    // Live-migration eligibility.  Called only on begin / estimate; the
    // mechanism is frozen on the record at begin time so later phases
    // don't need to re-check.
    var mechanism = ctx.req.params.migration_mechanism ||
        DEFAULT_MIGRATION_MECHANISM;
    if (VALID_MIGRATION_MECHANISMS.indexOf(mechanism) === -1) {
        callback(new restify.errors.InvalidArgumentError(
            'Invalid migration_mechanism: ' + mechanism +
            ' (must be one of ' + VALID_MIGRATION_MECHANISMS.join(', ') + ')'));
        return;
    }
    if (mechanism === 'live') {
        if (vm.brand !== 'bhyve') {
            callback(new restify.errors.PreconditionFailedError(
                'Live migration requires brand=bhyve (got ' + vm.brand + ')'));
            return;
        }
        // Pass-through PCI devices can't migrate — their host-specific
        // BDF and IOMMU group don't exist on the destination.  Reject
        // up-front rather than failing deep in the data plane.
        if (Array.isArray(vm.ppt) && vm.ppt.length > 0) {
            callback(new restify.errors.PreconditionFailedError(
                'Live migration not supported for VMs with PCI passthrough'));
            return;
        }
    }
...
function migrateVm(req, res, next) {
    ...
    vasync.pipeline({arg: {}, funcs: [
        ...
        function do_get_package(ctx, cb) {
            ...
            var errs = [];
            // On success, this will set vm.package to be the package object.
            common.validatePackageValues(req.app.papi, vm, errs,
                    function _validatePkgCb(err) {
                if (err) {
                    cb(err);
                    return;
                }
                ...
                assert.object(vm.package, 'vm.package should be set by ' +
                    'common.validatePackageValues() call');
                cb();
            });
        },
        ...
        function abort_with_no_target_server_uuid(ctx, cb) {
            ...
            // allocated, which means we simply delete the migration record.
        ...
        function start_create_new_record(ctx, cb) {
            ...
            if (!record) {
                record = {
                    ...
                    id: uuid.v4(),
                    // Frozen on begin; inherited by every later phase
                    // so operators don't re-specify and so the
                    // workflows dispatch consistently.
                    mechanism: ctx.mechanism || DEFAULT_MIGRATION_MECHANISM,
                    num_sync_phases: 0,
...
function deleteVmMigration(req, res, next) {
    assert.object(req, 'req');
    assert.object(req.app, 'req.app');
    assert.object(req.app.moray, 'req.app.moray');
    assert.object(req.log, 'req.log');
    ...
    req.app.moray.getVmMigrationByVmUuid(req.params.uuid, onLoadMigration);
    ...
    function onLoadMigration(err, migration) {
        ...
        'Deleting migration record');

        req.app.moray.delVmMigration(migration.id,
                function _delMigrationCb(delErr, etag) {
            if (delErr) {
                next(delErr);
                return;
            }

            delete MIGRATION_RECORD_FOR_VM_UUID[migration.vm_uuid];
...
function translateVmMigration(migration) {
    ...
    var exposedFields = [
        ...
        'state',
        'vm_uuid',
        // Live-migration additions.  Safe to expose on cold migrations
        // too — the fields are simply absent when not set, and they
        // give tooling (e.g. vmm-migrate-viz) enough to connect
        // directly to the per-CN agents' SSE streams and the guest's
        // reachability probe without a second round-trip to CNAPI.
        'mechanism',
        'source_server_uuid',
        'target_server_uuid',
        'source_admin_ip',
...
function storeMigrationRecord(req, res, next) {
    ...
    // For abort and rollback - at the end of a successful action delete
    // the migration record.
    if (record.state === 'successful' &&
            (record.phase === 'abort' || record.phase === 'rollback')) {
        req.app.moray.delVmMigration(record.id,
                function _delMigrationCb(err, etag) {
            if (err) {
                next(err);
...
module.exports = {
    ...
    deleteVmMigration: deleteVmMigration,
